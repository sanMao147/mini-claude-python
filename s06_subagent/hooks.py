...
def context_inject_hook(query: str):
    print(f"\033[90m[HOOK] {WORKSPACE_DIR}\033[0m")
    return None
...
def permission_hook(tool_name: str, args: dict):
    ...
    if tool_name in ("write_file", "edit_file") and _is_outside_workspace(args.get("path", "")):
...
def large_output_hook(tool_name, args, output):
    if len(output) > 100000:
        print(f"\033[33m[HOOK] ⚠ {tool_name} 输出: {len(output)} 字符\033[0m")
    return None
