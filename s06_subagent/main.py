...
SYSTEM_PROMPT = (
    ...
    "可用工具: bash, read_file, write_file, edit_file, glob, todo_write, task\n\n"
    "对于复杂任务：先用 todo_write 制定计划，再将独立子任务用 task 工具委托给子 Agent。\n"
    "子 Agent 会在隔离的上下文中完成任务并返回摘要。\n"
)
...
def agent_loop(messages: list[dict]):
    while True:
        ...
        if response["finish_reason"] != "tool_calls":
            ...
            if force_continue:
                messages.append({"role": "user", "content": str(force_continue)}); continue
...
def main():
    print("=" * 50)
    print("  s06: Subagent — 子 Agent 上下文隔离")
    print("  7 个工具: bash/read/write/edit/glob/todo_write/task")
    print("=" * 50)
