"""s09 持久化记忆系统 — 存储/加载/写入/整理四子系统

存储: .memory/ 下 Markdown + YAML frontmatter
加载: MEMORY_INDEX 常驻 System Prompt；select_relevant_memories() 用 LLM 侧查询选最多 5 条（关键词降级）
写入: 每轮 stop_reason != "tool_calls" 时 extract_memories() 自动提取
